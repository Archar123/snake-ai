...
class SnakeEnv(gym.Env):
    ...
    def get_action_mask(env):
        ...
        for action in range(env.action_space.n):
            ...
            if (row, col) in snake_list or row < 0 or col < 0 or row >= env.board_size or col >= env.board_size:
                mask[action] = False

        return mask
        
class ActionMasker(gym.Wrapper):
    def __init__(self, env, get_action_mask):
        super().__init__(env)
        self.get_action_mask = get_action_mask

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        return obs, {"action_mask": self.get_action_mask(self.env)}

    def step(self, action):
        obs, reward, done, truncated, info = self.env.step(action)
        return obs, reward, done, truncated, {"action_mask": self.get_action_mask(self.env), **info}